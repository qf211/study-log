...
- **fixture 分级**：`conn`（**session 级**——整个测试会话只建一次连接与表）+ `client`（**function 级**——每条用例清表），统一放在 `web/conftest.py` 共享
- **分组运行**：`pytest -m smoke`（9 条关键路径，提交前拿快速反馈）/ `pytest -m contract`（2 条"需求未定义、钉住当前行为"的用例）
