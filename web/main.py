...
def get_records():
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, date, topic, minutes, done FROM study_log")
    rows = cur.fetchall()
    conn.close()
...
def get_records_by_date(date: str):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM study_log WHERE date = ?", (date,))
    rows = cur.fetchall()
    conn.close()
    return [dict(row) for row in rows]
...
def get_record(records_id: int):
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT id, date, topic, minutes, done FROM study_log WHERE id = ?",
                (records_id,))
    row = cur.fetchone()
    conn.close()
    if row is None:
...
def get_stats():
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT COUNT(*) AS total_count, SUM(minutes) AS total_minutes FROM study_log")
    row = cur.fetchone()
    conn.close()
    return dict(row)
...
def update_records(record_id: int, record: UpdateRecord):
    ...
    cur.execute("UPDATE study_log SET date = ?,topic = ?, minutes = ?, done = ? WHERE id = ?",(record.date, record.topic, record.minutes, record.done, record_id))
    conn.commit()
...
def update_record_form(record_id: int, date: str = Form(...), topic: str = Form(...), minutes: int = Form(...)):
    ...
    if old is None:
        conn.close()
        raise HTTPException(status_code=404, detail='记录不存在')
    cur.execute("UPDATE study_log SET date = ?,topic = ?, minutes = ? WHERE id = ?",(date, topic, minutes, record_id))
    conn.commit()
    conn.close()
...
def list_page():
    conn = get_conn()
    cur = conn.cursor()
    cur.execute("SELECT * FROM study_log ORDER BY id")
    rows = cur.fetchall()
    conn.close()
    record_lines = []
    for row in rows:
